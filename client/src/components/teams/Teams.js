...
class Teams extends Component {
  constructor(props) {
    super(props);
    this.state = {
      myTeams: props.teams.myTeams,
    };
  }

  componentWillMount() {
    this.props.getTeams(this.props.auth.user.id);
  }

  componentWillReceiveProps(nextProps, nextContext) {
    if (this.props.teams.myTeams !== nextProps.teams.myTeams) {
      this.setState({
        myTeams: nextProps.teams.myTeams,
      });
    }
  }

  openNav = () => {
    document.querySelector(".team").style.marginLeft = "8rem";
  };

  closeNav = () => {
    document.querySelector(".team").style.margin = "0";
  };
  ...
  render() {
    ...
    return (
      <div>
        <NavbarHome />
        <div className="teams" style={{ marginTop: "60px" }}>
          <nav
            className="navbar"
            id="nav"
